/* Datihan global page loader + realtime bridge */
(function(){
  'use strict';
  ...
  style.textContent=`
    ...
    #datihan-page-loader{position:fixed;inset:0;z-index:2147483647;display:grid;place-items:center;background:#f1f0ec;color:#171717;font-family:"Inter",monospace;opacity:1;visibility:visible;transition:opacity .22s ease,visibility .22s ease}
    ...
    @keyframes datihanLoaderMove{0%{transform:translateX(-110%)}100%{transform:translateX(330%)}}
    @media(prefers-reduced-motion:reduce){#datihan-page-loader{transition:none}#datihan-page-loader .loader-line::after{animation:none;transform:translateX(120%)}}
  `;
  ...
  let finished=false;
  function finish(){if(finished)return;finished=true;root.classList.remove('datihan-loading');loader.classList.add('is-done');window.setTimeout(()=>loader.remove(),260)}
  function afterResources(){const fonts=document.fonts?.ready;if(fonts&&typeof fonts.then==='function')fonts.catch(()=>{}).then(()=>window.setTimeout(finish,180));else window.setTimeout(finish,220)}
